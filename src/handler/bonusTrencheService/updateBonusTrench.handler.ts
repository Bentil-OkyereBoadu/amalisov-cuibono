...
export class UpdateBonusTrancheHandler {
  ...
  public async updateBonusTranche(
    ...
    if (bonusTranche.Status) await UPDATE(BonusTranche, ID).with({Status});
    if (bonusTranche.Status === "Open"){
      const data = {
     ...
     if(targets.length > 0){
      ...
      return req.reject(400, `BonusTranche cannot be updted while the status is ${bonusTranche.Status}`)
    }
